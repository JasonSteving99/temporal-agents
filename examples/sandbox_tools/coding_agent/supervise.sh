...
# Supervises the server the agent starts, for EITHER backend: run by boot.sh (E2B's
# post_create_cmd), or as the Daytona snapshot's ENTRYPOINT.
#
...
#
# Tailnet joining is NOT here: it lives in tailscale_up.sh, which the E2B backend runs as its
# post_create_cmd before handing off to this script (see boot.sh). Keeping them separate means this
# file is purely "supervise the agent's server", usable by either backend.
#
# Contract:
...
#   * Both live INSIDE the project dir (/home/user/project on E2B, /home/daytona/project on Daytona —
#     boot.sh exports the E2B paths; the defaults below are the Daytona ones) so the agent can create
#     them with its
...
# hangs until the platform SIGKILLs us — slowing every stop/start.
