...
`bash`/`read`/`write`/`edit`/`grep`/`glob` tools execute inside an isolated **Daytona cloud sandbox**.
...
examples/
  ...
  Dockerfile.sandbox-coding-agent  # the Daytona snapshot the tools run in (built by `build-sandbox`)
